import RF
from RF_NTT import polynomial
from RF_NTT import polynomial_sum3_performance
...
def time_tree(newick):
    tree = Tree(newick)
    t0 = time.time()
    sum3_dt = polynomial_sum3_performance(tree, tree.get_leaves().__len__() + 1)[1]
    tf = time.time()
    return tf - t0, sum3_dt


performance_test_directory = '/Users/maryam/Desktop/Research/RF_improvment/FinalCodes/Frank/RF-master/TreePerformanceTest/'
...
for key in treeset_performances.keys():
    ...
    new_filename = '/Users/maryam/Desktop/Research/RF_improvment/FinalCodes/Frank/RF-master/performance_results_NTT/M' + key[4:] + '.csv'
